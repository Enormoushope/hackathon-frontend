...
const getItemReactions = async (itemId: string): Promise<ItemReaction[]> => {
  try {
    const res = await apiClient.get(`/reactions/items/${itemId}`);
    return res.data;
  } catch {
    return [];
  }
...
const addReaction = async (itemId: string, userId: string, reactionType: string): Promise<ItemReaction | null> => {
  try {
    const res = await apiClient.post('/reactions', { itemId, userId, reactionType });
    return res.data;
  } catch {
    return null;
  }
...
const removeReaction = async (itemId: string, userId: string, reactionType: string): Promise<boolean> => {
  try {
    await apiClient.delete('/reactions', { data: { itemId, userId, reactionType } });
    return true;
  } catch {
    return false;
  }
